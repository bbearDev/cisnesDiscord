...
export interface Command {
  ...
  /**
   * 응답 전에 디스코드 REST 를 부를 수 있는 명령은 `true` 로 둔다. **버튼이든 슬래시든 같다.**
   *
   * ★ 상호작용은 **3초 안에** 첫 응답이 가야 한다. DB 만 만지는 명령은 그 안에 끝나지만
   *   `/인증채널` 은 패널을 **게시하고 나서** 결과를 답하고, `인증` 버튼은 역할 재부여로
   *   `addRole` 을 부를 수 있다. 조립부(`discord/interactions.ts`)가 이 값을 보고 먼저
   *   "생각 중" 을 보낸 뒤 고쳐 쓴다 — REST 를 타는데 이 값이 없으면 사용자는
   *   "애플리케이션이 응답하지 않음" 을 본다.
   */
