...
export const CALL_TIMEOUT_MS: Readonly<Record<OutboundCall, number>> = {
  ...
  'rss-poll': 5_000,
  /**
   * ★★ **응답 시간 분포를 직접 재서 정한 값이다** (2026-09-14, 같은 요청 5회):
   *
   *   ```
   *   20.3s → 503 Transient error
   *   17.2s → 202
   *    0.5s → 202
   *    9.3s → 202
   *   20.3s → 503 Transient error
   *   ```
   *
   *   `pubsubhubbub.appspot.com` 은 구글이 App Engine 위에 올려 둔 오래된 허브이고,
   *   0.5초에서 20초까지 요동친다. 20초를 넘기면 503 으로 떨어진다.
   *   5초로는 5회 중 1회, 15초로는 2회만 건진다. **30초라야 분포를 덮는다.**
   *
   * ★★ 값을 올리는 두 번째 이유: 15초에서는 20초짜리 응답이 잘려 **`timeout` 으로만
   *   기록됐다.** 허브가 실제로 503 을 줬다는 사실이 로그 어디에도 남지 않아, 원인을
   *   보려면 사람이 손으로 curl 을 쏴야 했다. 30초면 `http 503` 이 그대로 찍힌다.
   *
   * ★ 실패가 특히 비싼 이유는 조용하기 때문이다 — 구독이 성립하지 않을 뿐 봇은 정상
   *   기동하고 업로드는 RSS 폴백으로 굴러간다. 유일한 표시가
