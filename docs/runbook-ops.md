...
### 8-d. ★ 라이브 공지에 그림이 없을 때 — `none` 은 두 경우가 섞여 있다
...
그림이 안 보일 때는 **수신 로그의 `image` 칸**을 먼저 본다 (`라이브 웹훅` 로그 줄):

```bash
journalctl --user -u cisnesdiscord -n 200 | grep '라이브 웹훅'
```

| `image` | 뜻 | 누가 확인 |
|---|---|---|
| `live` | 썸네일이 실렸다 — 정상 | — |
| `channel` | 썸네일이 없어 프로필로 내려갔다. **방송 시작 직후엔 정상** | — |
| `none` | 키가 아예 오지 않았다 → **아래 참조** | 이상하면 chzzkbot 쪽 |
| `dropped` | 값은 왔는데 우리 검사가 버렸다 | **우리 쪽. 원문 주소를 상류에 전달한다** |

**★★ `none` 은 우리 쪽에서 더 쪼갤 수 없다.** 두 경우가 섞인다:

- **(a)** 치지직이 애초에 안 줬다 — 정상이고 방송 시작 직후에 흔하다
- **(b)** 치지직이 줬는데 **chzzkbot 의 자리표시자 가드가 버렸다** — 형식이 바뀐 것이고
  사람이 알아야 하는 사고

(b) 여도 우리에게는 키가 오지 않으므로 (a) 와 똑같이 보인다. **구분은 chzzkbot 로그에만 남는다.**
방송이 한참 진행 중인데 `none` 이 계속 나오면 그쪽을 본다:
...
**`dropped` 는 반대로 chzzkbot 로그에 아무것도 남지 않는다.** 정의상 상류 가드를 통과한 값이
우리 검사(URL 파싱 · `http(s)` · 자리표시자)에 걸린 경우라, **우리 로그가 유일한 기록**이다.
이때는 원문 주소를 상류에 넘긴다 — 상류는 지금 자리표시자만 검사하고 파싱·프로토콜은 보지
않으므로, 그 값이 상류 가드를 늘릴 근거가 된다.
